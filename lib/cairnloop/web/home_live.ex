defmodule Cairnloop.Web.HomeLive do
  ...
  def render(assigns) do
    ...
    ~H"""
    <.cl_shell current={:home} destinations={Cairnloop.Web.Nav.destinations(@dashboard_path)}>
      <.cl_page title="Welcome back" subtitle="What needs you today?" width="wide">
          ...
          <.cl_hero job="Work the queue" count={@open_count}>
            <:detail>
              ...
              <%= if @resolved_count > 0 and not @resolved_count_unavailable? do %>
                <.link navigate={DashboardPath.to(@dashboard_path, "/inbox?status=resolved")} class="cl-text-small cl-text-muted">
            ...
            <:cta_slot>
              ...
              <.link navigate={DashboardPath.to(@dashboard_path, "/inbox")}><.cl_button variant="primary" size="lg">Open inbox</.cl_button></.link>
            </:cta_slot>
          </.cl_hero>
        <% end %>
        ...
        <div class="cl-home-grid cl-mt-5">
          <.cl_stat
            ...
            href={DashboardPath.to(@dashboard_path, "/knowledge-base/gaps")}
            cta="Review gaps"
          />
          <.cl_stat
            job="Audit trail"
            ...
            href={DashboardPath.to(@dashboard_path, "/audit-log")}
            cta="View audit log"
          />

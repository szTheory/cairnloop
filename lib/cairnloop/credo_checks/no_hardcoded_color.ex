# Only define this custom Credo check when Credo is actually loadable. Credo is a
# `only: [:dev, :test]` dependency of THIS library, so it is absent whenever the library is
# compiled as a dependency of another app (every adopter, and the bundled example app's test
# build) — and `use Credo.Check` would otherwise hard-fail that compile. Guarding keeps the
# check fully functional for `mix credo` in the library's own dev/test (where Credo is present)
# while letting the library compile cleanly everywhere else. The ExUnit brand-token gate
# (brand_token_gate_test.exs) remains the authoritative CI source of truth regardless.

defmodule Cairnloop.Web.InboxLiveTest do
  ...
  describe "triage row orientation" do
    test "renders queue orientation and per-row customer/activity context" do
      assigns =
        build_assigns(
          conversations: [
            %Cairnloop.Conversation{
              ...
              status: :open,
              host_user_id: "trailmark_customer_001",
              updated_at: ~N[2026-01-01 12:00:00]
            }
          ]
        )
      ...
      assert html =~ "Work the queue first."
      assert html =~ "Customer trailmark_customer_001"
      assert html =~ "Needs operator decision"
      assert html =~ "Updated"
    end
  end
  ...
  describe "normalize_status/1" do
    ...
    test "garbage does NOT raise ArgumentError (no String.to_existing_atom path)" do
      ...
      result =
        InboxLive.normalize_status(
          "a_very_long_random_value_that_has_never_been_an_atom_#{:rand.uniform(999_999)}"
        )
  ...
  describe "handle_params/3" do
    ...
    test "status=resolved sets @status = :resolved and loads the filtered list" do
      ...
      {:noreply, socket} =
        InboxLive.handle_params(%{"status" => "resolved"}, "/inbox?status=resolved", socket)
    ...
    test "status=garbage sets @status = nil and loads the unfiltered list" do
      ...
      {:noreply, socket} =
        InboxLive.handle_params(%{"status" => "garbage"}, "/inbox?status=garbage", socket)

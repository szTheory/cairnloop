defmodule Cairnloop.Web.HomeLiveTest do
  ...
  describe "HOME-02a recover-resolved sub-line" do
    test "resolved sub-line with /inbox?status=resolved present when resolved_count > 0" do
      html =
        rendered_to_string(
          Cairnloop.Web.HomeLive.render(assigns(%{open_count: 3, resolved_count: 4}))
        )
    ...
    test "resolved sub-line ABSENT when resolved_count == 0" do
      html =
        rendered_to_string(
          Cairnloop.Web.HomeLive.render(assigns(%{open_count: 3, resolved_count: 0}))
        )
  ...
  describe "D-06 count unavailable signal" do
    ...
    test "safe_count/1 |> split/1: a real count is reported honestly (NOT {0, true})" do
      assert {7, false} =
               Cairnloop.Web.HomeLive.safe_count(fn -> 7 end) |> Cairnloop.Web.HomeLive.split()

      assert {0, false} =
               Cairnloop.Web.HomeLive.safe_count(fn -> 0 end) |> Cairnloop.Web.HomeLive.split()
    end

    test "safe_count/1 |> split/1: a genuine error is the ONLY path to {0, true}" do
      assert {0, true} =
               Cairnloop.Web.HomeLive.safe_count(fn -> raise "boom" end)
               |> Cairnloop.Web.HomeLive.split()

      assert {0, true} =
               Cairnloop.Web.HomeLive.safe_count(fn -> throw(:nope) end)
               |> Cairnloop.Web.HomeLive.split()
    end
  end

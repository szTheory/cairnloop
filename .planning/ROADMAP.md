...
- â **vM009 Retrieval-First Support Answers & Search Ops** â Phases 1â8 (shipped 2026-05-21)
- â **vM010 KB AI Maintenance** â Phases 9â12 (shipped 2026-05-23)
...
- ð§ **vM014 Adoption Proof** â Phases 27â32 (active, kicked off 2026-05-27)

## Current Milestone: vM014 Adoption Proof

**Goal:** A reasonable adopter clones cairnloop, runs `mix setup` in the example app, opens two browser tabs, walks the full Jobs-To-Be-Done lifecycle live, and the same path is locked into CI â closing the 15% adopter-surface gap that remains after vM013, with zero churn to sealed primitives.
...
- [x] **Phase 27: Realistic Demo Fixtures** â Replace the lonely 1-conversation seed with a JTBD-spanning fixture set that exercises the M008 substrate end-to-end. (completed 2026-05-27)
...
2. **Phase 28 (chat ingress)** lands second because the golden-path smoke (31) and the JTBD walkthrough (32) both depend on the customer-ingress path actually working â the existing mock would let the smoke test pass against a fiction.
...
4. **Phase 30 (KB polish + SEC)** bundles the two `editor.ex` / `suggestion_review.ex`-shaped threats because they touch the same files as the editorial polish work â splitting would force a 7th phase. The domain-layer threats (T-10-10/T-10-12/T-10-13) graduate to vM015.
...
**Goal:** An adopter who runs `mix setup` in the example app lands in a populated dashboard that already exercises every Jobs-To-Be-Done state â the example self-tests the M008 retrieval substrate on first boot rather than greeting them with one lonely conversation.
**Depends on:** Nothing (foundation phase for vM014).
**Requirements:** FIX-01, FIX-02, FIX-03, FIX-04.
**Success Criteria** (what must be TRUE):

  1. Operator opens example app after `mix setup` and the inbox shows 12â16 conversations spanning `:new`, `:open`, `:awaiting_customer`, and `:resolved` with realistic operator + customer messages and ContextProvider snippets â not the previous 1-conversation lonely demo.
  ...
  4. `SuggestionReview` LiveView shows at least 1 `ArticleSuggestion` in `:ready_for_review` state with citation-backed `proposed_markdown` â real review work available immediately, no manual setup.

**Plans:** 8/8 plans complete

  - [x] 27-01-PLAN.md â Skeleton seeds.exs rewrite: builder shells + idempotency helper + Oban drain wiring + sealed-enum reconciliation table in header.
  - [x] 27-02-PLAN.md â `CairnloopExample.DemoContextProvider` module + headless test + config.exs wire (FIX-01 ContextProvider snippets; runs parallel to 27-01 in Wave 1).
  - [x] 27-03-PLAN.md â `build_articles/0`: 5 articles via KnowledgeBase facade + article-5 multi-revision progression v1âarchivedâv2 (FIX-02 articles).
  - [x] 27-04-PLAN.md â `build_conversations/1`: 16 conversations Ã 4 JTBD-derived cohorts with 3â6 messages each, brand-voice bodies (FIX-01 conversations).
  - [x] 27-05-PLAN.md â `build_gaps/1`: 3 GapCandidates + RetrievalGapEvents + memberships, all operator-scoped (FIX-03).
  - [x] 27-06-PLAN.md â `build_suggestion/2`: 1 ArticleSuggestion :ready + 2 evidence rows + companion ReviewTask via `ensure_review_task_for_suggestion/2` (FIX-04).
  - [x] 27-07-PLAN.md â Final wiring of `SeedRun.run/0` orchestrator + adopter-facing IO summary (FIX-02 substrate self-test driven by drain).
  - [x] 27-08-PLAN.md â Integration test `seeds_test.exs` pinning FIX-01..FIX-04 row counts + Oban drain non-empty chunks + idempotency (tagged `:requires_postgres`).

**UI hint:** yes

#### Phase 28: Customer `/chat` Wired to Real Ingress

**Goal:** The two-tab demo (operator inbox + customer `/chat`) proves a real customerâoperatorâcustomer round trip through the host-owned channel layer, replacing the mock `Process.send_after` bot reply with the same `WidgetChannel` path adopters will use.
...
**Success Criteria** (what must be TRUE):
  ...
  2. Customer types a message in `/chat`, it pushes through `WidgetChannel`, lands in an operator-side inbox conversation, and operator's reply broadcasts back into the customer's `/chat` LiveView via PubSub â no mock `Process.send_after(self(), :bot_reply, 1000)` path remains anywhere in `chat_live.ex`.
...
**Plans:** 3/3 plans complete
  ...
  - [x] 28-01-PLAN.md â Chat-facade foundation: add `create_customer_conversation/1` + `ingest_widget_message/2`, additive `reply_to_conversation/4` post-commit broadcast (OQ-1), `ConversationLive` + `InboxLive` PubSub handle_info clauses, and `Cairnloop.PubSub` started in the example app supervisor (Pitfall 1).
  - [x] 28-02-PLAN.md â Channel + worker rewire: `WidgetChannel.join("widget:lobby", ...)` creates the conversation via the new facade, `handle_in("new_message", ...)` reads conversation_id from `socket.assigns` (T-M001 input-trust mitigation), `ProcessMessage` rewritten with unique-option header + multi-clause `perform/1` preserving the EmailWebhookPlug stub (Pitfall 2 / OQ-2).
  - [x] 28-03-PLAN.md â Endpoint mount + ChatLive rewrite + README: `/widget` socket mount on the example endpoint, full `chat_live.ex` rewrite with colocated `WidgetChat` JS hook + UI-SPEC compliance + role-dedup handle_info (Pitfall 7), additive `Cairnloop.Chat.get_message/1` read facade, README Â§Two-Tab Demo block verbatim from UI-SPEC Â§3.
...
**Success Criteria** (what must be TRUE):
  ...
  1. `examples/cairnloop_example/assets/css/app.css` imports the canonical `:root` block from `prompts/cairnloop.css` (~30 semantic + ~15 primitive tokens) and the Tailwind `@theme` block extends them â replacing the previous 4-token + 6-raw-`--cl-*` placeholder.
  ...
  4. A negative-grep gate runs in the test lane and fails the build if `grep -r 'var(--cl-[a-z-]*, #' lib/cairnloop/web/` returns anything â the contract holds across future edits.

**Plans:** 3/3 plans complete

  - [x] 29-01-PLAN.md â BRAND-01: land canonical `:root` + `@theme` + dark overrides + `--cl-on-primary` alias in `examples/cairnloop_example/assets/css/app.css` (verbatim from `prompts/cairnloop.css`; replaces the 4-token stub).
  - [x] 29-02-PLAN.md â BRAND-02 + BRAND-04: drop hex fallbacks across 4 sealed render files (`inbox_live.ex` + `conversation_live.ex` + `search_modal_component.ex` + example app `chat_live.ex`), rename `--cl-error` â `--cl-danger` in `chat_live.ex`, refresh `inbox_live.ex` moduledoc, land negative-grep gate test `test/cairnloop/web/brand_token_gate_test.exs`.
  - [x] 29-03-PLAN.md â BRAND-03: re-pin all 6 hex-fallback assertions across 3 integration test files (`approval_footer_live_test.exs` + `tool_execution_outcome_live_test.exs` + `bulk_recovery_live_test.exs`) to bare `var(--cl-<token>)` form with closing-paren strictness.

**UI hint:** yes

#### Phase 30: KB Editorial Polish + T-10-09 / T-10-11 Closure

**Goal:** The four KB routes feel like one coherent editorial surface, operators can create / inspect / review without context-switching between unrelated layouts, calm reason-forward copy holds across affordances, and two of the five outstanding vM010 SECURITY threats close via an auditable handoff marker â without churning the sealed render code structure.
...
**Success Criteria** (what must be TRUE):
  ...
  1. Operator navigates between `KnowledgeBase.Index`, `KnowledgeBase.Editor`, `SuggestionReview`, and the KB gap surface and sees a single shared editorial nav shell â no mid-task context-switch between unrelated layouts.
  ...
  4. `Cairnloop.KnowledgeAutomation.EditorHandoff.verify!/2` requires a `manual_edit_opened_at` timestamp marker on the handoff record; `KnowledgeBase.Editor` refuses to preload `proposed_markdown` from a bare URL `suggestion_id` parameter â only via the handoff marker (closes T-10-09 + T-10-11; T-10-10 / T-10-12 / T-10-13 remain deferred to vM015 per assessment thread).

**Plans:** 4 plans

  - [x] 30-01-PLAN.md â EditorHandoff double-layer gate (decode/1, sign/5 marker opts, 3-step verify!/2) + record_editor_handoff/2 + get_gap_candidate/2 + list_articles/1 + manual_edit_changeset/2 + pure gate tests (SEC-01, SEC-02, KB-02/KB-03 facades).
  - [x] 30-02-PLAN.md â NavComponent.kb_nav/1 shared editorial nav + ReviewTaskPresenter 3-variant calm copy + pure nav/presenter tests (KB-01 component, KB-04).
  - [x] 30-03-PLAN.md â Index (list_articles read, New article button, nav) + Editor (gap sidebar, mount rescue, nav) + updated knowledge_base_live_test.exs marker/rescue fixtures (KB-01/KB-02/KB-03, SEC-02 consumer).
  - [x] 30-04-PLAN.md â SuggestionReview (record_editor_handoff + marker sign + nav) + Gaps nav + ConversationLive open_manual_draft gate fix + suggestion_review_test (KB-01, SEC-01 minting side).
...
**Success Criteria** (what must be TRUE):
  ...
  1. `test/integration/golden_path_test.exs` (using `Phoenix.LiveViewTest`) drives the full JTBD round trip â seed customer message â operator inbox sees â ConversationLive + cmd+k search + citation chip â approve AI draft â tool proposal approve â `ToolExecutionWorker` `:success` â resolve â `Outbound.trigger/2` from sidebar â multi-select bulk recovery â `BulkEnvelope` row created + per-recipient `OutboundWorker` jobs enqueued â and passes against real Postgres + pgvector.
  2. `test/integration/widget_channel_test.exs` (using `Phoenix.ChannelTest`) drives the customer-ingress side: customer message join â push through `WidgetChannel` â PubSub broadcast â operator-side delivery â proving the CHAT-01/CHAT-02 wiring end-to-end.
  3. Both new tests are registered in the `mix test.integration` lane (dockerized Postgres + pgvector) and run green in CI on every push â no Wallaby, no PhoenixTest dep, no browser-driver flake.
...
**Success Criteria** (what must be TRUE):
  ...
  1. Root `README.md` leads with `mix cairnloop.install` (the shipped Igniter task at `lib/mix/tasks/cairnloop/install.ex`), not the previous `{:cairnloop, "~> 0.1.0"}` snippet â the install path adopters should actually use is the first one they see.
...
- [x] 32-04-PLAN.md — mix.exs ExDoc wiring (extras + groups_for_extras + package :files) + mix docs/hex.build verification (DOC-03; Wave 2)
...
<details>
<summary>â vM013 Support-Triggered Outbound Lifecycle (Phases 22â26) â SHIPPED 2026-05-27</summary>

- [x] Phase 22: Outbound Foundation & Persistence (1/1 plan) â completed 2026-05-26
- [x] Phase 23: Delivery & Scheduling Engine (1/1 plan) â completed 2026-05-26
- [x] Phase 24: Individual Outbound UI (1/1 plan) â completed 2026-05-26
- [x] Phase 25: Bulk Selection & Fan-out (3/3 plans) â completed 2026-05-27
- [x] Phase 26: Observability & Polish (3/3 plans) â completed 2026-05-27
...
<details>
<summary>â vM012 Public Release & MCP Write Surface (Phases 18â21) â SHIPPED 2026-05-26</summary>

- [x] Phase 18: Release Gate & Hex.pm Publish (3/3 plans) â completed 2026-05-25
- [x] Phase 19: Example Phoenix App (1/1 plan) â completed 2026-05-26
- [x] Phase 20: MCP OAuth Seam (2/2 plans) â completed 2026-05-26
- [x] Phase 21: MCP Write Tools (1/1 plan) â completed 2026-05-26
...
<details>
<summary>â vM011 AI Tool Governance & MCP Integration (Phases 13â17) â SHIPPED 2026-05-25</summary>

- [x] Phase 13: Governed Tool Contract & Proposal Records (3/3 plans) â completed 2026-05-24
- [x] Phase 14: Operator Timeline & Preview Surface (4/4 plans) â completed 2026-05-24
- [x] Phase 15: Approval State Machine & Oban Resume (5/5 plans) â completed 2026-05-25
- [x] Phase 16: First Approved Write Path & Telemetry (3/3 plans) â completed 2026-05-25
- [x] Phase 17: Optional Evidence Lane & Read-Only MCP Seam (2/2 plans) â completed 2026-05-25
...
<details>
<summary>â vM010 KB AI Maintenance (Phases 9â12) â SHIPPED 2026-05-23</summary>

- [x] Phase 9: Gap Candidate Discovery (3/3 plans) â completed 2026-05-22
- [x] Phase 10: Citation-Backed Draft Suggestions (4/4 plans) â completed 2026-05-22
- [x] Phase 11: Review-Gated KB Updates (4/4 plans) â completed 2026-05-23
- [x] Phase 12: In-Thread Quick Fix & Ops Closure (4/4 plans) â completed 2026-05-23
...
<details>
<summary>â vM009 Retrieval-First Support Answers & Search Ops (Phases 1â8) â SHIPPED 2026-05-21</summary>

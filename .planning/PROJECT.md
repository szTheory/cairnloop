...
---
*Last updated: 2026-05-27 — vM014 Phase 28 complete: customer chat wired to real ingress. `WidgetSocket` mounted at `/widget`, `ChatLive` rewritten with real Phoenix channel + PubSub operator-reply push, `ProcessMessage` multi-clause perform/1 with Oban unique idempotency, `Chat` facade (`create_customer_conversation/1`, `ingest_widget_message/2`, `get_message/1`) fully wired. Two-tab demo documented in example README. Headless tests: 22 chat, 3 channel, 2 process_message, 8 chat_live (697 root tests). Human UAT pending (live two-tab browser test).*

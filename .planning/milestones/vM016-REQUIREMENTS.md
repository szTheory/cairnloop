# Requirements Archive: vM016 Operator UI/UX Iteration
...
- [x] **MOTION-01**: Restrained CSS-only brand motion is applied to the v1 surfaces (hero count < 180ms; evidence-rail reveal 260ms; gate state-flip 180ms; list enter staggered ≤5; toast enter/exit) using transform + opacity only — never on reply-send, keystrokes, count ticks, or layout properties. Route-line / marker-travel motion remains deferred to AMOTION-01.
- [x] **MOTION-02**: `prefers-reduced-motion` is honored live — movement is removed while comprehension cross-fades are kept.

### Seed Enrichment & Verification (SEED / VERIFY)

- [x] **SEED-01**: The demo seed fully expresses every screen's states: varied audit timestamps (~14d) with real reasons plus a rejected and a deferred event; rejected/deferred/published KB suggestions; ≥1 draft-state article; 1–2 masked MCP tokens; and a second/higher-risk governed-action tool type.
- [x] **VERIFY-01**: Light + dark screenshots are regenerated for the touched screens via the Playwright pipeline, per-screen visual acceptance is recorded, and motion is verified live (dark mode is the tell that drift remediation themed correctly).
- [x] **VERIFY-02**: `mix test` is green including the hardened brand-token gate and `golden_path_test.exs`; the integration suite is validated with `PGPORT=5432 PGUSER=postgres PGPASSWORD=postgres MIX_ENV=test mix test.integration` plus the quality lane before claiming green.
...
| MOTION-02 | Phase 44 | Complete |
| SEED-01 | Phase 45 | Complete |
| VERIFY-01 | Phase 45 | Complete |
| VERIFY-02 | Phase 45 | Complete |
...
*Last updated: 2026-06-26 — Phase 45 seed enrichment, screenshot regeneration, and verification sweep complete.*

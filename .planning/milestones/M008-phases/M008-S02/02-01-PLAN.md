...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    Use the Ecto.Multi core CRUD pattern identified in `lib/cairnloop/automation.ex` as an analog for transactional safety.
  </action>
  <verify>
    <automated>mix test test/cairnloop/knowledge_base_test.exs</automated>
  </verify>
  <done>Earmark is installed and Context logic successfully manages draft vs published states per immutability rules, matching the automation context analog patterns.</done>
</task>

<task type="auto" tdd="true">
  <name>Task 2: Build LiveView Editor and Index</name>
  <files>lib/cairnloop/web/knowledge_base_live/index.ex, lib/cairnloop/web/knowledge_base_live/editor.ex, test/cairnloop/web/knowledge_base_live_test.exs</files>
  <behavior>
    - Test 1: Editor renders preview side-by-side using Earmark when Markdown is input.
    - Test 2: Editor handles debounced phx-change events properly to avoid excessive parsing.
  </behavior>
  <action>
    Create `KnowledgeBaseLive.Index` to list articles.
    Create `KnowledgeBaseLive.Editor` for authoring. Follow the side-by-side layout pattern and the imports/state management pattern from `lib/cairnloop/web/conversation_live.ex` as identified in `PATTERNS.md`.
    ...
    Include the `Error Handling` shared pattern from `lib/cairnloop/web/conversation_live.ex` to gracefully handle flash errors (e.g. `{:error, _failed_operation, _failed_value, _changes_so_far} -> ... put_flash(socket, :error, ...)`) when context executions fail.
    Write tests in `test/cairnloop/web/knowledge_base_live_test.exs` to verify the side-by-side preview and debounced events.
  </action>
  <verify>
    <automated>mix test test/cairnloop/web/knowledge_base_live_test.exs</automated>
  </verify>
  <done>LiveViews exist with debounced input, Earmark preview rendering, wired context operations, correct analog patterns for error handling and layout, and passing tests.</done>
</task>
...
<task type="auto">
  ...
  <action>
    In `lib/cairnloop/router.ex`, add routes within the `:cairnloop_dashboard` live session block, following the route definition pattern identified in `PATTERNS.md` for `lib/cairnloop/router.ex`:
  ...
  <done>The Knowledge Base LiveViews are reachable via standard application routing matching the router analog pattern.</done>
</task>
